		...
		namespace battle
		{
			namespace fighters
			{
				void PlayerFighter::playTurn(Fighter& target)
				{
					...
					battle::HitDirection hitDirection{ askHitDirection() };

					Fighter::hit(target, hitDirection);
				}

				battle::HitDirection PlayerFighter::askHitDirection()
				{
					while (true)
					{
						...
						{
							this->getMessageDisplayer().display("Entered not valid hit direction. Please, try again.\n\n");
						}
					}
