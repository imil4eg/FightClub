		...
		namespace characterstuff
		{
			namespace abilities
			{
				struct Ability::Impl
				{
					...
					Impl(std::string name, Type type, int cost, int damage) : 
						m_name{ name }, m_type{ type }, m_cost{ cost }, m_damage{ damage }
					{
					}

					Impl(boost::uuids::uuid&& id, std::string&& name, Type type, int cost, int damage) : 
						m_id{std::move(id)},
				...
				Ability::Ability(std::string name, Type type, int cost, int damage) :
					pImpl{ std::make_unique<Impl>(name, type, cost, damage) }
				{
				}

				Ability::Ability(boost::uuids::uuid&& id, std::string&& name, Type type, int cost, int damage) :
